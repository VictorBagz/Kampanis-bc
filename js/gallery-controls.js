...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.gallery-item img').forEach(img => {
        img.addEventListener('error', function() {
            ...
            // Add a placeholder background and text
    ...
    console.log('Works Gallery:', worksGallery ? 'Initialized' : 'Failed');
    console.log('Products Gallery:', productsGallery ? 'Initialized' : 'Failed');
    
    // Force image loading for gallery items
    setTimeout(() => {
        document.querySelectorAll('.gallery-item img').forEach(img => {
            if (img.complete && img.naturalHeight !== 0) {
                img.style.opacity = '1';
            } else {
                // Try to reload the image if it failed to load
                const src = img.src;
                img.src = '';
                img.src = src;
            }
